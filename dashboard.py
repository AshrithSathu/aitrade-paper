...
def worker(engine,view):
    while True:
        ...
        try:
            ...
            with lock:
                publish(engine,view);view['busy']=False;idle.notify_all()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            minutes,engine,view=self.context();path=urlsplit(self.path).path
            ...
            with lock:
                s=engine.state
                if path=='/api/codex/login':
                    if any(not e.state['paused'] or e.future for e in engines.values()) or login['running']:raise ValueError('Pause and wait for the current review or login')
                ...
                elif path=='/api/pause':engine.pause()
                elif path=='/api/start':
                    ...
                    if str(body.get('market_minutes'))!=minutes:raise ValueError('Settings belong to another tab')
                    ...
                    p.atomic_json(engine.data_dir/'settings.json',body)
                    settings_by[minutes]=body;engine.config=checked;engine.epoch+=1
...
if __name__=='__main__':
    ...
    for minutes in ('15','5'):
        ...
        state=json.loads((directory/'state.json').read_text()) if (directory/'state.json').exists() else p.initial_state(settings['balance'])
        if state.get('venue')!='polymarket':raise ValueError('Wrong paper account venue')
        engine=p.Engine(state,settings,feed,data_dir=directory);engine.pause()
        view={'busy':False,'error':None,'updated':None}
        engines[minutes]=engine;views[minutes]=view;settings_by[minutes]=settings
        p.atomic_json(engine.state_path,state);p.atomic_json(directory/'settings.json',settings);publish(engine,view)
    for minutes,engine in engines.items():threading.Thread(target=worker,args=(engine,views[minutes]),daemon=True).start()
