...
Checks: `bun run test`; `python3 -m py_compile railway_start.py`. Deploy only the eight runtime files via a temporary source directory, never the local data or credentials. Run `railway up <source-dir> --path-as-root --project 75e81864-86d2-4c44-a4fe-4e2563a5b5df --environment production --service paper --detach --json`.
...
Infrastructure is managed in `.railway/railway.ts`. Use `bun install`, `railway config plan`, then `railway config apply` for infrastructure changes. The imported app, PostgreSQL and volumes match the existing environment. Secrets use `preserve()` and stay on Railway. The legacy `railway.json` and service config-file override were removed. Application uploads contain Dockerfile, .dockerignore, railway_start.py, paper_trader.py, dashboard.py, dashboard.html, dashboard.js and chainlink.mjs; infrastructure config is applied separately.
