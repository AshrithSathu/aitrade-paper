...
An entry response uses a 30-second lifetime and includes its maximum ask, maximum adverse BTC/USD movement and maximum adverse selected-contract ask increase. The backend calculates fractional contracts from current NAV, the configured allocation percentage, available cash, market minimums, depth and the hard dollar/contract caps. It reads the current WebSocket-backed snapshot after Codex returns and requires the contract actually purchased, including a reversed contract, to retain positive fee-adjusted edge. Movement further in the predicted direction and a cheaper ask remain valid. These bounds come from the AI for that market; account limits and data freshness remain non-negotiable safety checks. WAIT uses zero values and consumes the market's single review.
