...
An entry response uses a 30-second lifetime and includes its maximum ask, maximum adverse BTC/USD movement and maximum adverse selected-contract ask increase. Quantity must remain within the contract limit, and quantity times the ask plus fees must remain within the dollar trade cap. The backend reads the current WebSocket-backed snapshot after Codex returns and rejects a reversal against the chosen outcome or a more expensive contract outside those bounds. Movement further in the predicted direction and a cheaper ask remain valid. These bounds come from the AI for that market; account limits and data freshness remain non-negotiable safety checks. WAIT uses zero values and consumes the market's single review.
