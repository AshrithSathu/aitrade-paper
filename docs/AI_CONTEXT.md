...
An entry response uses a 30-second lifetime and includes its maximum ask, maximum adverse BTC/USD movement and maximum adverse selected-contract ask increase. The backend calculates fractional contracts from current NAV, the configured allocation percentage, available cash, market minimums, depth and the hard dollar/contract caps. It reads the current WebSocket-backed snapshot after Codex returns and rechecks fee-adjusted edge in normal mode. Reverse mode deliberately flips the validated AI decision after the response, while retaining freshness, drift, price, depth, NAV and drawdown checks. Movement further in the executed direction and a cheaper ask remain valid. WAIT uses zero values and consumes the market's single review.
