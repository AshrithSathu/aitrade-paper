...
const form=$('settings');let loaded=false,inFlight=false,settings=null,selectedMinutes='15',actionBusy=false;
function tabsDisabled(value){$('tab5').disabled=value;$('tab15').disabled=value}
for(const minutes of ['5','15'])$('tab'+minutes).onclick=()=>{if(inFlight||actionBusy)return;selectedMinutes=minutes;loaded=false;settings=null;$('saved').textContent='';$('start').disabled=true;$('pause').disabled=true;for(const m of ['5','15']){$('tab'+m).setAttribute('aria-selected',String(m===minutes));$('tab'+m).className=m===minutes?'':'secondary'}refresh()};
async function action(path,body={}){
 if(actionBusy)return;actionBusy=true;tabsDisabled(true);
 try{const r=await fetch('/api/'+path+'?minutes='+selectedMinutes,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}),d=await r.json();if(!r.ok)throw Error(d.error);$('saved').textContent=path==='settings'?'Limits saved.':'Done.';await refresh()}
