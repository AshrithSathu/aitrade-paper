...
let selectedMinutes='15',actionBusy=false;const loadedSettings=new Set();const loadedModes=new Set();let latest=null;
function tabsDisabled(value){$('tab5').disabled=value;$('tab15').disabled=value}
for(const minutes of ['5','15'])$('tab'+minutes).onclick=()=>{if(actionBusy)return;selectedMinutes=minutes;$('saved').textContent='';for(const m of ['5','15']){$('tab'+m).setAttribute('aria-selected',String(m===minutes));$('tab'+m).className=m===minutes?'':'secondary'}if(latest)render(latest)};
async function action(path,body={},minutes=selectedMinutes){
 ...
 catch(e){$(path==='settings'?'saved'+minutes:'saved').textContent=e.message}
...
for(const minutes of ['5','15']){
 ...
 form.onsubmit=e=>{e.preventDefault();if(!latest)return;const body={...latest.accounts[minutes].settings,...Object.fromEntries(new FormData(form)),assets:['BTC'],market_minutes:minutes};body.daily_loss=String(-Math.abs(Number(body.daily_loss)));action('settings',body,minutes)};
}
...
function render(update){
  ...
  for(const minutes of ['5','15']){
   const account=update.accounts[minutes],form=$('settings'+minutes),limits=account.settings;
   if(!loadedSettings.has(minutes)){for(const k of ['balance','max_trade','size','daily_loss'])form.elements[k].value=k==='daily_loss'?Math.abs(Number(limits[k])):limits[k];loadedSettings.add(minutes)}
   const used=Math.max(0,-Number(account.account.realized_pnl)),limit=Math.abs(Number(limits.daily_loss));
