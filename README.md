...
AI reviews now receive up to 24 hours of recorded TWAP in 15-minute bars, alongside the latest hour of one-minute candles and 1/5/15/30/60-minute signals. This context is refreshed once per minute; gaps and actual coverage are shown. Longer history is accumulated, not fabricated or backfilled. Starting a run sets a persisted 12-hour deadline by default; the UI/API accepts 0.25 to 168 hours. The engine pauses AI/entries at expiry, while existing positions continue to official settlement. Boot remains paused.

The AI input is a deliberate structured briefing: duplicate provider metadata is excluded and order books are represented by top-10 levels plus whole-book depth totals, while exact live/opening prices, top liquidity, fees, limits and signal definitions remain. Historical tables identify units/columns and round USD prices to cents. Profit targets use settled profit since run start divided by run-start equity; zero disables the target. Time/profit/loss stops pause AI and new entries without changing hold-to-settlement behavior.

### BTC market tabs
The dashboard offers independent 5-minute and 15-minute paper accounts, each starting at $1,000 with its own limits and timed run. Existing account files remain the 15-minute account; the new account uses data/polymarket/5m. API calls select ?minutes=5 or ?minutes=15 (default 15 for compatibility). Tabs only switch the view. A shared Chainlink collector records BTC prices once; each mode uses its own opening timestamp, market tokens, books and contract history. Broad 24-hour context remains useful in both modes. Five-minute reviews run at minute 1; 15-minute reviews at minute 3. Both use Astra Low via Codex 0.154.0, with a 25-second review timeout and plugins/apps disabled for these snapshot-only calls.
