...
- Reviews start 5 seconds after opening for both durations, with a 60-second dispatch window (5–under 65 seconds) to wait for validated data. Codex returns a short-lived conditional plan; the current BTC price and selected contract ask must remain inside its buffered limits before entry. Missed windows, WAIT decisions and errors skip that market.
...
- Each trade uses the configured percentage of current NAV, capped by maximum dollars and contracts. The backend calculates fractional contracts. Normal mode retains its fee-adjusted edge check; reverse mode deliberately flips the validated AI decision afterward.
- Maximum NAV drawdown and profit targets stop new entries. There is no per-position early-exit stop-loss.
